...
class PostEditSidebarControl extends events.EventTarget {
    ...
    _syncExpanderTitles() {
        this._notesExpander.title = `Notizen (${this._post.notes.length})`;
    ...
    _evtFeatureClick(e) {
        e.preventDefault();
        if (confirm("Bist du sicher, dass du diesen Beitrag an der Startseite anheften möchtest?")) {
            this.dispatchEvent(
    ...
    _evtDeleteClick(e) {
        e.preventDefault();
        if (confirm("Bist du sicher, dass du diesen Beitrag unwiederuflich löschen möchtest?")) {
            this.dispatchEvent(
    ...
    _evtPasteNotesClick(e) {
        ...
        const text = window.prompt(
            "Bitte gib die exportierten Notizen ein:"
        );
        if (!text) {
            return;
        }
