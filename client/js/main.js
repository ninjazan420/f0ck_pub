...
// Funktion zum Anwenden der gespeicherten Lautstärke und Speichern der neuen Lautstärke
function applyVolumeSetting(video) {
    ...
    mutations.forEach(function (mutation) {
        if (mutation.type === 'childList') {
            ...
            videos.forEach((video) => {
                // Überprüfen, ob das Volume-Event bereits angehängt ist, um doppelte Events zu vermeiden
                if (!video.dataset.volumeEventAttached) {
                    applyVolumeSetting(video);
                    video.dataset.volumeEventAttached = true; // Kennzeichnung, dass das Event bereits angehängt wurde
                }
            });
        }
    });
